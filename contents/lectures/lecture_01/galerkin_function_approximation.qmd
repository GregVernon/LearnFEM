...
Specifically, we want to find the polynomial function, of a given degree, that most closely represents the function over a finite domain, in the $L^2$ sense.
...
%| echo: false
figure
fplot( target_fun, double( domain ), LineWidth=4, Color="w" )
```
...
\begin{align}
    ...
    \begin{bmatrix}
        ...
        \vdots & \ddots & \vdots \\
        \Inner{\Matrix{T}_m}{\Matrix{F}_1} & \cdots & \Inner{\Matrix{T}_m}{\Matrix{F}_n}
    \end{bmatrix}
    %
    ...
    \begin{bmatrix}
        ...
        \vdots \\
        \Inner{\Matrix{T}_m}{\Matrix{F}_n} \Vector{f}_1 + \cdots + \Inner{\Matrix{T}_m}{\Matrix{F}_n} \Vector{f}_n \\
    \end{bmatrix}
\end{align}
```
...
Recognizing that our target function is approximated in our finite-dimensional solution space as a linear combination of the basis functions, we can rewrite the right-hand side as
...
figure
hold on
fplot( target_fun, double( domain ), LineWidth=4, Color="w",     DisplayName="$g(x)$" )
fplot( approx_fun, double( domain ), LineWidth=3, SeriesIndex=1, DisplayName="$\tilde{g}(x)$" )
```
...
EmitLatexEqn( "L^2 \ \mathrm{error} \ = " + latex( vpa( l2_error, 4 ) ) )
```
...
```{matlab}
%| output: false
function [u, M, F, basis, d] = ScalarProjection( fun, sol_space )
...
for p = degree
    sol_space.degree = p;
    approx_fun = ScalarProjection( fun, sol_space );
    l2_error(p+1) = vpa( ComputeL2Error( fun.target_fun, approx_fun, fun.domain, "exact" ), 32 );
end
```
...
figure
cmap = colormap( "lines" );
plot( degree, l2_error, LineWidth=2, Marker="o", MarkerFaceColor=cmap(1,:), MarkerEdgeColor="w" )
ax = gca;
xlabel( "Degree" )
ylabel( "$L^2$ Error" )
ax.YScale = "log";
...
for ii = 1 : length( basis_functions )
    ...
    for p = degree 
        basis = PolynomialBasisFunction( basis_name, p, x, sol_space.domain );
        D = int( basis .* transpose( basis ),sol_space.domain );

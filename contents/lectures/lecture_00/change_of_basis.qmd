...
\begin{align}
    ...
    %
    \Inner{\Matrix{M}_A}{\sum_{B}^{n}{\Vector{d}_B \Matrix{M}_B}} &= \Inner{\Matrix{M}_A}{\sum_{B}^{n}{\Vector{c}_B \Matrix{N}_B}} \\
    %
    \sum_{B}^{n}\Vector{d}_B\Inner{\Matrix{M}_A}{\Matrix{M}_B} &= \sum_{B}^{n}\Vector{c}_B\Inner{\Matrix{M}_A}{\Matrix{N}_B}, \quad A = 1, \ldots, n \\
\end{align}
```
...
\begin{equation}
    \begin{bmatrix}
        ...
        \vdots & \ddots & \vdots \\
        \Inner{\Matrix{M}_n}{\Matrix{M}_1} & \cdots & \Inner{\Matrix{M}_n}{\Matrix{M}_n}
    \end{bmatrix}
    %
    ...
    \begin{bmatrix}
        ...
        \vdots & \ddots & \vdots \\
        \Inner{\Matrix{M}_n}{\Matrix{N}_1} & \cdots & \Inner{\Matrix{M}_n}{\Matrix{N}_n}
    \end{bmatrix}
    %
...
If it helps, you can think of the right-hand side as a known vector, $\Vector{b} = \Matrix{C}\Vector{c}$.
Then all we are really doing is solving the linear system $\Matrix{D}\Vector{d}=\Vector{b}$.
Alternatively, we can evaluate the matrix-matrix product $\ChangeBasisOp{N}{M} = \Matrix{D}^{-1} \Matrix{C}$, which we call the *change of basis operator*.
This is handy when we want to reuse the same basis change more than once.

```{=latex}
\begin{equation}
    \Vector{d} = \ChangeBasisOp{N}{M} \Vector{c}
\end{equation}
```
...
\begin{equation}
    \begin{bmatrix}
        ...
        \vdots & \ddots & \vdots \\
        \Inner{\Matrix{T}_n}{\Matrix{T}_1} & \cdots & \Inner{\Matrix{T}_n}{\Matrix{T}_n}
    \end{bmatrix}
    %
    ...
    \begin{bmatrix}
        ...
        \vdots & \ddots & \vdots \\
        \Inner{\Matrix{T}_n}{\Matrix{F}_1} & \cdots & \Inner{\Matrix{T}_n}{\Matrix{F}_n}
    \end{bmatrix}
    %
...
vector_color = cmap(1,:);
% Lighten toward white and use quiver() so this chunk stays on built-in Matlab plotting tools.
basis_color = vector_color + 0.25 * ( 1 - vector_color );
scaled_basis_color = vector_color + 0.50 * ( 1 - vector_color );

p{1} = quiver( 0, 0, double( U(1,1) * u(1) ), double( U(2,1) * u(1) ), 0, LineWidth=1.5, Color=scaled_basis_color, MaxHeadSize=0.4 );
p{2} = quiver( 0, 0, double( U(1,2) * u(2) ), double( U(2,2) * u(2) ), 0, LineWidth=1.5, Color=scaled_basis_color, MaxHeadSize=0.4 );
...
p{5} = quiver( 0, 0, double( U(1,1) ), double( U(2,1) ), 0, LineWidth=1.5, Color=basis_color, MaxHeadSize=0.5 );
p{6} = quiver( 0, 0, double( U(1,2) ), double( U(2,2) ), 0, LineWidth=1.5, Color=basis_color, MaxHeadSize=0.5 );
p{7} = quiver( 0, 0, double( u_vec(1) ), double( u_vec(2) ), 0, LineWidth=2.0, Color=vector_color, MaxHeadSize=0.35 );

vector_color = cmap(2,:);
basis_color = vector_color + 0.25 * ( 1 - vector_color );
scaled_basis_color = vector_color + 0.50 * ( 1 - vector_color );

p{8}  = quiver( 0, 0, double( V(1,1) * v(1) ), double( V(2,1) * v(1) ), 0, LineWidth=1.5, Color=scaled_basis_color, MaxHeadSize=0.4 );
p{9}  = quiver( 0, 0, double( V(1,2) * v(2) ), double( V(2,2) * v(2) ), 0, LineWidth=1.5, Color=scaled_basis_color, MaxHeadSize=0.4 );
...
p{12} = quiver( 0, 0, double( V(1,1) ), double( V(2,1) ), 0, LineWidth=1.5, Color=basis_color, MaxHeadSize=0.5 );
p{13} = quiver( 0, 0, double( V(1,2) ), double( V(2,2) ), 0, LineWidth=1.5, Color=basis_color, MaxHeadSize=0.5 );
p{14} = quiver( 0, 0, double( v_vec(1) ), double( v_vec(2) ), 0, LineWidth=2.0, Color=vector_color, MaxHeadSize=0.35 );
```
...
Within our mnemonic, $\Matrix{T} \equiv \Matrix{L}$, $\Matrix{F} \equiv \Matrix{M}$, $\Vector{f} \equiv \Vector{m}$, and the representation we wish to find is $\Vector{t} \equiv \Vector{\ell}$.

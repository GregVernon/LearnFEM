...
While some classes of functions are difficult to integrate, integration of polynomials is a solved problem, as a simple antiderivative rule exists for each monomial term:
...
And, because of the linearity properties of integration:
...
We can easily integrate a polynomial:
...
function [qp, weights] = LagrangeQuadrature( domain, num_qp, method )
    if num_qp == 1
        ...
        qp = linspace( domain(1), domain(2), num_qp );
    end
    x = sym( "x", "real" );
...
The sane oscillatory Runge phenomena that we saw occur with higher-order Lagrange interpolations means that high-order Lagrange quadrature schemes also suffer.

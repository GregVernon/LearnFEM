...
g(x) = sin( pi * x )^2;
tess_coeffs = g( tess_spline_space.vertices )
spline_coeffs =  transpose( R * transpose( tess_coeffs ) )

figure
subplot( 2, 1, 1 )
hold on
fplot( tess_coeffs * tess_spline.basis, double( domain ) )
fplot( g, double( domain ), Color="k" )
xline( double( vertices ), LineStyle=":" )

subplot( 2, 1, 2 )
hold on
fplot( spline_coeffs * spline.basis, double( domain ) )
fplot( g, double( domain ), Color="k" )
xline( double( vertices ), LineStyle=":" )

TL = sym( zeros( length( spline.basis ), length( tess_spline.basis ) ) );
for n = 1 : length( tess_spline.basis )
    TL(:, n) = subs( spline.basis, x, tess_spline_space.vertices(n) );
end
%for e = 1 : length( degree )
%    elem_domain = vertices(e:e+1);
%    elem_degree = degree(e);
...
%        TL(:, tess_basis_ids(n)) = subs( spline.basis, x, elem_nodes(n) );
%    end
%end
display( TL.' )

spline_coeffs =  TL * tess_coeffs.'
...
subplot( 2, 1, 2 )
hold on
fplot( TL * transpose( spline.basis ), double( domain ) )
fplot( g, double( domain ), Color="k" )
xline( double( vertices ), LineStyle=":" )

...
# Polynomial Roots
## Computing polynomial roots using linear algebra
...
$$
Which can be computed from any given polynomial by simply dividing by the leading coefficient.
...
The companion matrix is constructed from the monic polynomial as:
...
Recall that for any square matrix $\Matrix{A}$, its *characteristic polynomial* is defined as 
$$
\rho_{\Matrix{A}}(\lambda) \coloneqq \det(\Matrix{A} - \lambda \Matrix{I}).
$$

Recalling that $\det(\Matrix{A} - \lambda \Matrix{I}) = (\lambda_1 - \lambda)(\lambda_2 - \lambda)\cdots(\lambda_n - \lambda)$ it's clear that $\lambda_{\Index{i}}$ is an eigenvalue of $\Matrix{A}$ *if and only if* $\rho_{\Matrix{A}}(\lambda_{\Index{i}}) = 0$. 
In other words, the eigenvalues of a matrix are the roots of its characteristic polynomial.
...
## Example: Polynomial roots via eigenvalues
...
%| echo: false
%| output: asis
EmitLatexEqn( "p(x) = " + string( latex( polynomial ) ) )
EmitLatexEqn( "\bar{p}(x) = " + string( latex( monic_polynomial ) ) )
EmitLatexEqn( "\Matrix{C} = " + string( latex( C ) ) )
EmitLatexEqn( "\lambda( \Matrix{C} ) \approx " + string( latex( transpose( round( eigvals, 5 ) ) ) ) )
```
...
## On the computation of eigenvalues

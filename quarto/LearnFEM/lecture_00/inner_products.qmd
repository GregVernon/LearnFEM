...
let
  ...
  p2 = Plots.plot( g, 0, 2π, label="\$g(x)\$" )
  p3 = Plots.plot( L2, 0, 2π, label="\$\\| f, g \\|_2 \$" )
  # Plots.plot( p1, p2, p3, layout=Plots.@layout([a;b;c]), link=:x, fmt=:png )
end
``` -->
...
f = sin( x )
g = sin( x )
L2 = SymbolicNumericIntegration.integrate( f * g, ( x, 0 // 1, 2π ); symbolic = true, detailed = false )
```

```{=latex}
\begin{equation}
  %\sideset{_{[0, \pi]}}{}{\Inner{\sin(x)}{\sin(x)}}
  %\prescript{\pi}{0}{\Inner{\sin(x)}{\sin(x)}}
  \Inner{\sin(x)}{\sin(x)}_{[0, \pi]}
\end{equation}
```

...
for degree = 0 : 3;
    ...
    origin_line = yline( 0, Alpha=1 );
    origin_line.Annotation.LegendInformation.IconDisplayStyle = "off";
...
for degree = 0 : 3;
    basis = BernsteinBasis( degree, x, domain );
    %expr = "\Matrix{B}^{" + num2str( degree ) + "}(x) = " + string( latex( expand( basis ) ) );
    expr = "\Matrix{B}^{" + num2str( degree ) + "}(x) = " + PolyLatex( basis );
    EmitLatexEqn( expr )
end
``` -->

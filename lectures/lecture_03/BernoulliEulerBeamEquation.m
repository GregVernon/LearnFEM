function [u, M, F, basis, d] = BernoulliEulerBeamEquation( basis_family, degree, domain, youngs_modulus, moment_inertia, displacement_constraint, slope_constraint, distributed_load, moment_load, shear_load, boundary_tol, penalty )
...
% function F = MomentBoundaryCondition( basis, domain, moment_load )
% F = subs( basis(1), symvar( basis(1) ), domain(2) ) * moment_load;
% end
...
% function F = ShearBoundaryCondition( basis, domain, shear_load )
% F = subs( basis(0), symvar( basis(0) ), domain(2) ) * shear_load;
% end
